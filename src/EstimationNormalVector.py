...
## method uses three points to calculate the normal vector
...
def show_normal_vector(source, radius=0.1, near_sample_num=15):
    ...
    for _, src_point in enumerate(source):
        ...
        if len(indices) >= point_num + 1:
            ...
            plate_pcd.points = o3d.utility.Vector3dVector(
                np.asarray(np_plate.reshape(-1, 3))
            )
...
def viewpoint_process(source, normal, viewpoint, inverse=False):
    ...
    for i in range(len(source)):
        ...
        p2 = normal[i]
        angle = np.arctan2(np.linalg.norm(np.cross(p1, p2)), np.dot(p1, p2.T))
        # viewpoint 와 normal vector간 각도 계산
        if inverse:
            if (angle < np.pi / 2) and (
                angle > -np.pi / 2
            ):  # viewpoint 와 normal vector간 각도가 +-90도 보다 작은
                normal[i] = -normal[i]
                # 법선 부호 뒤집기
        else:
            if (angle > np.pi / 2) or (
                angle < -np.pi / 2
            ):  # viewpoint 와 normal vector간 각도가 +-90도 보다 크면
                normal[i] = -normal[i]
                # 법선 부호 뒤집기
    return normal
